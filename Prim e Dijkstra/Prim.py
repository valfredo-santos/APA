...
# Faz a leitura do arquivo e retorna uma martriz de adjacencia
...
# Retorna o vertice com a menor chave que ainda não está na solução
...
# Imprime o resultado
def printResult(graph, parent, V):
        aux = 0
        print ('\nSolution:', end='')
        for i in range(1, V):
            print ('(' + str(parent[i]) + ',' + str(i) + ')', end='')
            aux += graph[i][parent[i]]
